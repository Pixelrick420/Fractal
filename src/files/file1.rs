...
fn __fractal_debug_init() {
    __FRACTAL_DBG_INIT.call_once(|| {
        let __f = OpenOptions::new().create(true).write(true).truncate(true).open("/home/theerttha/code/Fractal/src/files/file1.debug.jsonl").expect("cannot open fractal debug file");
...
pub fn fractal_fibonacci(mut fractal_n: i64) -> i64 {
    ...
    __fractal_debug_snapshot!("If", "fibonacci", 0, [], false, None::<&str>);
    if (fractal_n <= 1_i64) {
        __fractal_debug_snapshot!("Return", "fibonacci", 0, [], false, None::<&str>);
        return fractal_n;
    } else {
        __fractal_debug_snapshot!("Return", "fibonacci", 0, [], false, None::<&str>);
...
pub fn fractal_main() {
    ...
    __fractal_debug_snapshot!("ExprStmt", "main", 0, [], false, None::<&str>);
    {
        let mut fractal_i: i64 = 0_i64;
        while fractal_i < 3_i64 {
            __fractal_debug_snapshot!("For i", "main", 0, [], false, None::<&str>);
